...
class PluginNextoolLicenseValidator {
   ...
   public static function validateLicense(array $options = []) {
      ...
      if ($responseData === null) {
         ...
         // (resiliência ao ContainerAPI indisponível) em vez de degradar para FREE. O bloco de sucesso
         // abaixo (sync de catálogo / entitlement anti-pirataria) NÃO roda aqui -- a graça só preserva
         // o direito já provado, sem aplicar alterações destrutivas offline.
    ...
    * confia em token não verificado. Sem token (servidor antigo), no-op. (G10: Config direto.)
    */
   protected static function persistVerifiedEntitlement(?string $clientId, array $responseData): void {
      ...
      try {
         Config::setConfigurationValues(PluginNextoolEntitlementToken::ENTITLEMENT_CONTEXT, [
            'entitlement_token'           => $token,
            'entitlement_token_exp'       => (int) ($claims['exp'] ?? 0),
            'entitlement_token_synced_at' => date('Y-m-d H:i:s'),
         ]);
      } catch (Throwable $e) {
         Toolbox::logInFile('plugin_nextool', 'LicenseValidator: falha ao persistir entitlement_token - ' . $e->getMessage());
      }
   }
